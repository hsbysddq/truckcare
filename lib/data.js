...
// validationDaily, fuelWeekly — dan lib/analytics.js tidak perlu diubah.
// Deterministik (pseudoRandom, tanpa Date.now) supaya aman untuk SSR.
const ANALYTICS_SOURCE_DAYS = 180;
const ANALYTICS_FUEL_WEEKS = 26;
...
export function getAnalyticsSource() {
  ...
  const fuelWeekly = plates.map((plate) => {
    const base =
      fuelConsumptionByTruck.series.find((s) => s.plateNumber === plate)?.values ??
      [100, 100, 100, 100];
    const avg = base.reduce((a, b) => a + b, 0) / base.length;
    const anomalyWeeks = FUEL_ANOMALY_WEEKS_AGO[plate] ?? [];
    const points = [];
    for (let w = ANALYTICS_FUEL_WEEKS - 1; w >= 0; w -= 1) {
      const end = new Date(ANALYTICS_END_DATE);
      end.setDate(end.getDate() - w * 7);
      const start = new Date(end);
      start.setDate(start.getDate() - 6);
      const anomaly = anomalyWeeks.includes(w);
      const noise = 0.9 + pseudoRandom((w + 1) * 11.3 + plateSeed(plate)) * 0.2;
      points.push({
        weekStart: isoDate(start),
        weekEnd: isoDate(end),
        liters: Math.round(avg * noise * (anomaly ? 0.3 : 1)),
        anomaly,
      });
    }
    return { plateNumber: plate, points };
  });
