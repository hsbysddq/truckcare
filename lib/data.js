...
].map((complaint) => ({ ...complaint, decisionSource: "agent" }));

export function getComplaints() {
  return complaints;
}

// Pengemudi contoh: satu per truk, id stabil supaya bisa dipilih di form jadwal.
...
export function getSchedules(now = new Date()) {
  ...
  trucks.forEach((truck, ti) => {
    ...
    for (let dayOffset = -SCHEDULE_DAYS_BACK; dayOffset <= SCHEDULE_DAYS_AHEAD; dayOffset += 1) {
      ...
      if (!forced && !plant && pseudoRandom(seed * 1.3) > 0.55) continue;
