...
export async function getTrucksShape() {
  ...
  const [trukMentah, posisiMentah] = await Promise.all([
...
export async function getComplaintsShape() {
  const rows = await baca(
    "pengaduan",
    "?select=id,plat,tanggal,jam,deskripsi,status,foto_url,created_at&order=created_at.desc&limit=100",
    ANON
  );
  return (rows || []).map((r) => ({
    id: r.id,
...
async function tulis(tabel, method, params, badan) {
