...
export async function getAnalyticsSourceLive({ rangeDays = 90 } = {}) {
  ...
  try {
    const [ngebut, aduan, validasi] = await Promise.all([
      panggilRpc("analitik_ngebut", { p_hari: hari, p_batas: BATAS_NGEBUT_KPJ }, KUNCI),
...
async function agregatDariBarisMentah({ sejakMs, sejakTanggal, KUNCI }) {
  ...
  for (const p of posisi || []) {
    if (!p?.ts || !p.truk_id) continue;
    const tanggal = tanggalWIB(p.ts);
    const jam = jamWIB(p.ts);
    const k = `${tanggal}|${jam}|${p.truk_id}`;
    const g = ngebutMap.get(k) ?? { tanggal, jam, truk_id: p.truk_id, jumlah: 0, lat: 0, lon: 0 };
    g.jumlah += 1;
    g.lat += Number(p.lat) || 0;
    g.lon += Number(p.lon) || 0;
    ngebutMap.set(k, g);
  }
...
//   ngebut   [{tanggal, jam, truk_id, jumlah, lat, lon}]
