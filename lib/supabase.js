...
export async function getComplaintsShape() {
  const rows = await baca(
    "pengaduan",
    "?select=id,plat,tanggal,jam,deskripsi,status,alasan,diputuskan_oleh,foto_url,created_at&order=created_at.desc&limit=100",
    ANON
  );
  return (rows || []).map((r) => {
    const decisionSource = sumberKeputusan(r);
    return {
    id: r.id,
    judul: r.deskripsi,
    lokasi: "",
