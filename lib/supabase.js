...
export async function getAnalyticsSourceLive({ rangeDays = 90, debug = false } = {}) {
  if (!URL || !ANON) throw new Error("Env Supabase belum diisi di .env.local");
  ...
  // Jejak query yang BENAR-BENAR dijalankan + jumlah baris (untuk
  // /api/analytics?debug=1). Zona waktu: batas bawah dihitung dalam UTC
  // (kolom ts timestamptz), kunci tanggal hasil dalam WIB.
  const jejak = {
    jendelaHari: hari,
    sekarangUtc: now.toISOString(),
    hariIniWib: tanggalWIB(now.toISOString()),
    sejakUtc: new Date(sejakMs).toISOString(),
    sejakTanggalWib: sejakTanggal,
    ambangKph: BATAS_NGEBUT_KPJ,
    kunci: SERVICE ? "service_role" : "anon",
    jalur: null,
    query: [],
  };
  const catat = (nama, detail, hasil, error) => {
    jejak.query.push({ nama, ...detail, baris: Array.isArray(hasil) ? hasil.length : null, error: error ?? null });
    return hasil;
  };

  const [truk, bbm] = await Promise.all([
    daftarTrukCache(true).then((rows) => catat("trucks (cache 60 detik)", { url: "GET /rest/v1/trucks?select=id,plat,nama,tipe,status&status=eq.aktif&order=nama&limit=200" }, rows)),
    baca(
      "fuel_readings",
  ...
  jejak.trukAktif = (truk || []).map((t) => t.plat);

  let agregat;
  try {
    const rpc = async (nama, args) => {
      try {
        return catat(`rpc ${nama}`, { url: `POST /rest/v1/rpc/${nama}`, args }, await panggilRpc(nama, args, KUNCI));
      } catch (e) {
        catat(`rpc ${nama}`, { url: `POST /rest/v1/rpc/${nama}`, args }, null, e.message);
        throw e;
      }
    };
    const [ngebut, aduan, validasi] = await Promise.all([
      ...
      rpc("analitik_ngebut", { p_hari: hari }),
      rpc("analitik_pengaduan_harian", { p_hari: hari }),
      rpc("analitik_validasi_harian", { p_hari: hari }).catch(() => []),
    ]);
    jejak.jalur = "rpc (supabase/analitik-agregat.sql)";
    jejak.insidenAgregat = (ngebut || []).reduce((t, g) => t + (Number(g.jumlah) || 0), 0);
    jejak.insidenTrukAktif = (ngebut || []).filter((g) => (truk || []).some((t) => t.id === g.truk_id)).reduce((t, g) => t + (Number(g.jumlah) || 0), 0);
    agregat = { ngebut, aduan, validasi };
  } catch {
    jejak.jalur = "baris mentah (fungsi SQL tidak tersedia)";
    agregat = await agregatDariBarisMentah({ sejakMs, sejakTanggal, KUNCI, catat });
    jejak.insidenAgregat = agregat.ngebut.reduce((t, g) => t + g.jumlah, 0);
    jejak.insidenTrukAktif = agregat.ngebut.filter((g) => (truk || []).some((t) => t.id === g.truk_id)).reduce((t, g) => t + g.jumlah, 0);
  }
  const source = shapeAnalyticsSource({ truk, ...agregat, bbm, now });
  jejak.speedingEventsSetelahFilterTruk = source.speedingEvents.reduce((t, e) => t + (e.count ?? 1), 0);
  if (debug) source.debug = jejak;
  return source;
}

// Jalur cadangan: baris mentah dibatasi rentang waktu, dikelompokkan ke
// bentuk yang sama dengan hasil fungsi SQL.
async function agregatDariBarisMentah({ sejakMs, sejakTanggal, KUNCI, catat = (n, d, h) => h }) {
  const sejakIso = new Date(sejakMs).toISOString();
  const qPos = `?select=truk_id,lat,lon,kecepatan,ts&kecepatan=gt.${BATAS_NGEBUT_KPJ}&ts=gte.${encodeURIComponent(sejakIso)}&order=ts.desc&limit=5000`;
  const qAduan = `?select=plat,tanggal,status,created_at,decided_at,decided_by&deleted_at=is.null&tanggal=gte.${sejakTanggal}&limit=5000`;
  const qRuns = `?select=started_at,finished_at&finished_at=gte.${encodeURIComponent(sejakIso)}&limit=5000`;
  const [posisi, aduanMentah, runs] = await Promise.all([
    baca("positions", qPos, ANON).then((rows) => catat("positions", { url: `GET /rest/v1/positions${qPos}` }, rows)),
    baca("pengaduan", qAduan, ANON).then((rows) => catat("pengaduan", { url: `GET /rest/v1/pengaduan${qAduan}` }, rows)),
    SERVICE
      ? baca("agent_runs", qRuns, SERVICE).then((rows) => catat("agent_runs", { url: `GET /rest/v1/agent_runs${qRuns}` }, rows)).catch(() => [])
      : Promise.resolve([]),
  ]);
  void KUNCI;
