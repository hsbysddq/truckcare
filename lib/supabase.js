...
export async function getAnalyticsShape() {
  ...
  const [aduan, ngebut, truk] = await Promise.all([
    ...
    baca("trucks", "?select=id,plat&status=eq.aktif&limit=200", ANON),
  ]);
...
// ---------- SUMBER ANALITIK LIVE (bentuk = getAnalyticsSource) ----------
...
export async function getAnalyticsSourceLive() {
  if (!URL || !ANON) throw new Error("Env Supabase belum diisi di .env.local");
  // Hanya truk aktif (trucks.status = 'aktif'), sama dengan halaman Armada.
  const [truk, aduan, posisi, runs] = await Promise.all([
    baca("trucks", "?select=id,plat,tipe&status=eq.aktif&limit=200", ANON),
    baca(
      "pengaduan",
      "?select=plat,tanggal,status,created_at,decided_at,decided_by&deleted_at=is.null&order=tanggal.desc&limit=5000",
      ANON
    ),
    baca(
      "positions",
      "?select=truk_id,lat,lon,kecepatan,ts&kecepatan=gt.80&order=ts.desc&limit=5000",
      ANON
    ),
    // agent_runs: durasi validasi agent (started_at -> finished_at). Tabel
    // tanpa policy anon; baca via service role, gagal -> [] (fallback di bawah).
    SERVICE
      ? baca(
          "agent_runs",
          "?select=started_at,finished_at&finished_at=not.is.null&order=started_at.desc&limit=5000",
          SERVICE
        ).catch(() => [])
      : Promise.resolve([]),
  ]);
  return shapeAnalyticsSource({ truk, aduan, posisi, runs });
}

// Bentuk sumber analitik (murni, tanpa I/O) dari baris tabel Supabase.
// Dipisah supaya bisa diuji tanpa DB. Bentuk keluaran = getAnalyticsSource()
// di lib/data.js; semua hitungan tetap di lib/analytics.js.
export function shapeAnalyticsSource({ truk, aduan, posisi, runs, now = new Date() }) {
  ...
  return {
    ...
    speedingEvents,
    validationDaily,
    fuelDaily: plates.map((plateNumber) => ({
      plateNumber,
      vehicleType: metaDariPlat[plateNumber]?.label ?? null,
      capacityLiters: metaDariPlat[plateNumber]?.tankLiters ?? null,
      points: [],
    })),
    manualEstimateMinutes: 15,
  };
