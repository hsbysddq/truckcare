...
export async function getAnalyticsSourceLive() {
  if (!URL || !ANON) throw new Error("Env Supabase belum diisi di .env.local");
  // fuel_readings bisa lebih dari 1000 baris (PostgREST memotong di 1000),
  // baca berhalaman supaya seluruh level solar terbaca.
  async function bacaSemua(tabel, select, order) {
    const semua = [];
    for (let offset = 0; ; offset += 1000) {
      const batch = await baca(
        tabel,
        `?select=${select}&order=${order}&limit=1000&offset=${offset}`,
        ANON
      );
  ...
  const [truk, aduan, posisi, runs] = await Promise.all([
    ...
    // Durasi validasi agent dari agent_runs (baca service role; gagal -> []
    // lalu dipakai selisih decided_at pengaduan di bawah).
    SERVICE
      ? baca(
          "agent_runs",
  ...
  const bbm = await bacaSemua("fuel_readings", "truck_id,tanggal,liters,refill", "tanggal.asc");
