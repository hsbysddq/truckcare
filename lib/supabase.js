...
export async function getTrucksShape() {
  ...
  const [trukMentah, posisiMentah, tripBerjalan, drv] = await Promise.all([
    ...
    baca("trips", "?select=truk_id,driver_id,asal,tujuan,mulai&status=eq.berjalan&order=mulai.desc&limit=200", ANON),
    baca("drivers", "?select=id,nama&limit=200", ANON),
  ]);
...
export async function getComplaintsShape({ includeDeleted = false } = {}) {
  const rows = await baca(
    "pengaduan",
    `?select=id,plat,tanggal,jam,deskripsi,status,alasan,diputuskan_oleh,decided_by,decided_by_name,decided_at,operator_note,operator_note_at,deleted_at,delete_reason,foto_url,created_at${
