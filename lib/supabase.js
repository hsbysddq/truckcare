...
export async function getAnalyticsSourceLive() {
  if (!URL || !ANON) throw new Error("Env Supabase belum diisi di .env.local");
  // PostgREST membatasi max-rows (default 1000) sehingga limit besar
  // di-clamp diam-diam; paginasi supaya seluruh baris terbaca.
  ...
  const complaintEvents = (aduan || [])
    .filter((r) => r?.tanggal)
    .map((r) => ({ date: r.tanggal, status: statusTren(r.status), avgSeconds: detikValidasi(r) }));
  ...
  const speedingEvents = (posisi || [])
    ...
    .map((p) => ({
      ...
      hour: jamWIB(p.ts),
      plateNumber: platDariId[p.truk_id] ?? p.truk_id,
    }));

  const perHari = {};
  for (const r of aduan || []) {
    if (!r?.decided_at) continue;
    const k = tanggalWIB(r.decided_at);
    if (!k || k === "Invalid Date") continue;
    if (!perHari[k]) perHari[k] = { total: 0, n: 0 };
    perHari[k].total += detikValidasi(r);
    perHari[k].n += 1;
  }
