...
const SELECT =
  "?select=id,truck_id,driver_id,origin,destination,planned_departure,planned_arrival,actual_departure,actual_arrival,status,cargo_type,notes,created_at,trucks(plat,tipe,status),drivers(nama)";
...
function fromRow(r) {
  return {
    ...
    // "Nama" truk = jenis armadanya (tidak ada lagi "Truk NN").
    truck_name: r.trucks ? truckTypeMeta(truckTypeFromTipe(r.trucks.tipe, r.trucks.plat)).label : null,
...
export async function listSchedules({ from, to } = {}) {
  if (supabaseReady()) {
    ...
    // Jadwal milik truk nonaktif tidak ditampilkan di mana pun.
    return (rows || []).filter((r) => !r.trucks || r.trucks.status !== "nonaktif").map(fromRow);
  }
  seed();
...
export async function createSchedule(input) {
  ...
  const row = {
    ...
    truck_name: truck?.vehicleType ?? null,
...
export async function updateSchedule(id, input) {
  ...
  memory.rows[index] = {
    ...memory.rows[index],
    ...pickWritable(input),
    ...(truck ? { plate_number: truck.plateNumber, truck_name: truck.vehicleType } : {}),
