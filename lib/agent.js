...
// truckPlate (opsional): plat truk yang sedang jadi konteks percakapan.
export async function sendMessageToAgent(userText, truckPlate = null) {
  try {
    const response = await fetch("/api/agent/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message: userText, truck: truckPlate || undefined }),
    });
    if (response.ok) {
      ...
      if (data?.text) {
        return {
          ...
          role: "agent",
          text: data.text,
          ...(data.toolTrace ? { toolTrace: data.toolTrace } : {}),
          ...(data.dataCard ? { dataCard: data.dataCard } : {}),
        };
      }
    }
  } catch {
