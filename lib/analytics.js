...
// Harga solar per liter untuk estimasi kerugian anomali — satu-satunya
// tempat angka ini didefinisikan; ubah di sini bila harga berubah.
...
export function buildAnalytics(source, filters = {}) {
  ...
  // Insiden ngebut dari sumber live sudah diagregasi per (hari, jam, truk)
  // dan membawa bobot `count`; data contoh satu baris = satu insiden.
  const bobot = (e) => Math.max(1, Number(e.count) || 1);
  const jumlahInsiden = (list) => list.reduce((s, e) => s + bobot(e), 0);
  ...
  // "Diselesaikan otomatis": persentase pengaduan yang diputuskan agent
  // (decidedBy dari kolom decided_by). Data lama tanpa decidedBy: yang tidak
  // berstatus perlu ditinjau dianggap selesai otomatis.
  const otomatis = (c) => (c.decidedBy !== undefined && c.decidedBy !== null ? c.decidedBy === "agent" : c.status !== "perluDitinjau");
  const autoPct = (list) =>
    list.length ? Math.round((list.filter(otomatis).length / list.length) * 100) : 0;
  ...
  const metrics = {
    ...
    speedingIncidents: {
      ...delta(jumlahInsiden(speedCur), jumlahInsiden(speedPrev)),
  ...
  for (const e of speedCur) countByPlate.set(e.plateNumber, (countByPlate.get(e.plateNumber) ?? 0) + bobot(e));
  const typeByPlate = new Map(source.fuelDaily.map((t) => [t.plateNumber, t.vehicleType ?? null]));
  const speedingRows = [...countByPlate.entries()]
    .map(([plateNumber, count]) => ({ plateNumber, count, vehicleType: typeByPlate.get(plateNumber) ?? null }))
    .sort((a, b) => b.count - a.count || a.plateNumber.localeCompare(b.plateNumber));
  const speedTotal = jumlahInsiden(speedCur);
  let cumulative = 0;
  let contributors = 0;
  ...
  // Liter terpakai per hari: selisih level (bukan hari pengisian); hari
  // anomali = besar penurunannya.
  const usedLiters = (p) => (p.refill ? 0 : Math.max(0, -p.deltaLiters));

  // Rata-rata seluruh armada per tanggal (level & konsumsi): garis pembanding.
  const fleetSum = new Map();
  for (const t of fuelInWindow) {
    for (const p of t.points) {
      const acc = fleetSum.get(p.date) ?? { level: 0, used: 0, n: 0 };
      acc.level += p.liters;
      acc.used += usedLiters(p);
      acc.n += 1;
      fleetSum.set(p.date, acc);
    }
  }
  const fleetAvgLevel = (date) => {
    const acc = fleetSum.get(date);
    return acc ? Math.round(acc.level / acc.n) : null;
  };
  const fleetAvgUsed = (date) => {
    const acc = fleetSum.get(date);
    return acc ? Math.round(acc.used / acc.n) : null;
  };

  const fuelTrucks = fuelInWindow
    .map((t) => {
      const normalUse = t.points.filter((p) => !p.refill && !p.anomaly && p.deltaLiters < 0);
      const avgDailyUseLiters = normalUse.length
        ? Math.round(normalUse.reduce((s, p) => s + usedLiters(p), 0) / normalUse.length)
        : 0;
      const points = t.points.map((p) => ({
        date: p.date,
        label: formatShort(p.date),
        liters: p.liters,
        fleetAvgLiters: fleetAvgLevel(p.date),
        usedLiters: usedLiters(p),
        fleetAvgUsedLiters: fleetAvgUsed(p.date),
        refill: p.refill,
        anomaly: p.anomaly,
        dropLiters: p.anomaly ? usedLiters(p) : 0,
        lossLabel: p.anomaly ? formatRupiah(usedLiters(p) * FUEL_PRICE_PER_LITER) : null,
      }));
      const anomalies = points
        ...
        .map((p) => ({
          date: p.date,
          label: p.label,
          labelLong: formatLong(p.date),
          dropLiters: p.dropLiters,
          liters: p.liters,
          lossRupiah: p.dropLiters * FUEL_PRICE_PER_LITER,
        }))
        .sort((a, b) => b.dropLiters - a.dropLiters);
      const lossRupiah = anomalies.reduce((s, a) => s + a.lossRupiah, 0);
      return {
        plateNumber: t.plateNumber,
        vehicleType: t.vehicleType ?? null,
        capacityLiters: t.capacityLiters,
        ...
        anomalies,
        lossRupiah,
        lossRupiahLabel: formatRupiah(lossRupiah),
      };
    })
  ...
  const totalLoss = fuelTrucks.reduce((s, t) => s + t.lossRupiah, 0);
  const worstFuel = fuelTrucks
    .filter((t) => t.anomalyCount > 0)
    .sort((a, b) => b.anomalies[0].dropLiters - a.anomalies[0].dropLiters)[0];
  const fuelByTruck = {
    trucks: fuelTrucks,
      ...
      fuelTrucks.length === 0
        ...
        : worstFuel
          ? fill(copy.charts.fuel.insight, {
              ...
              drop: worstFuel.anomalies[0].dropLiters,
              date: worstFuel.anomalies[0].labelLong,
              loss: formatRupiah(worstFuel.anomalies[0].lossRupiah),
              more:
                totalAnomalies > 1
                  ? fill(copy.charts.fuel.insightMoreSuffix, {
                      n: totalAnomalies,
                      totalLoss: formatRupiah(totalLoss),
                    })
                  : "",
            })
  ...
  for (const e of speedCur) {
    const jam = Math.max(0, Math.min(23, Number(e.hour) || 0));
    hourCounts[jam].count += bobot(e);
  }
  let peakFrom = 0;
  let peakSum = -1;
