...
export function buildAnalytics(source, filters = {}) {
  ...
  // ---- Konsumsi solar per truk (daftar + detail) ----
  const fuelTrucks = source.fuelWeekly
    .filter((t) => plateSet.has(t.plateNumber))
    .map((t) => {
      const points = t.points
        .filter((p) => inWindow(p.weekEnd, start, end))
        .map((p) => ({ week: formatShort(p.weekStart), liters: p.liters, anomaly: p.anomaly }));
      return {
